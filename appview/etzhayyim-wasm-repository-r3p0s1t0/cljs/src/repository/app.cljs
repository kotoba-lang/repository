(ns repository.app
  "etzhayyim-wasm-repository-r3p0s1t0 appview — reagent + re-frame, view built
  ...
  port adds no repository/git functionality that was not already there.
  ...
  itself copied from `cloud-itonami/app-tia`'s `tia-mcp-component/cljs`).

  Backend TypeScript is unaffected by this migration:
  `../src/app.ts` (the production XRPC dispatcher — git object model over
  Actor DID, ref dispatch, commit ES256 signature verify, FaaS build
  trigger) is untouched. The one other TypeScript file that lived under the
  deleted `svelte/` tree —
...
(def default-db
  {:title "Repository R3p0s1t0"
   :project "etzhayyim-project-repository"
   :name "etzhayyim-wasm-repository-r3p0s1t0"
   :kind "appview"
   :route-count 2
   :routes ["r3p0s1t0.etzhayyim.com/*" "repository.etzhayyim.com/*"]
   :vars ["AGENTGATEWAY_MCP_ROUTER_URL" "APP_ACTOR_HANDLE" "APP_CAPABILITIES"
          ...
          "INTERFACES_REQUIRES"]
   :xrpc? true
   :relative-path "appview/etzhayyim-wasm-repository-r3p0s1t0/cljs/src/repository/app.cljs"})
